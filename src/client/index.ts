...
import { MsgType, StorageType } from '../constant'
...
function clientRun(context: ExtensionContext): [LanguageClient, Dove] {
  ...
  // 创建语言服务器并启动客户端
  client = new LanguageClient('languageServerApiMate', 'Language Server Api Mate', serverOptions, clientOptions)
  ...
  client.onReady().then(async () => {
    ...
    dove.subscribe(Client_Server_MsgTYpe.GIVE_INFO_FROM_PATH__SERVER, (apiPath: string) => {
      // 顺便检查更新
      checkVersion()
