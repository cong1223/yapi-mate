...
import { Command, ContextEnum, GIT_REMOTE_URL, MsgType, SideBarView, StorageType } from './constant'
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  // const wvController = new WebviewController(context);
  /** 初始化storage */
  storage.init(context)

  /** 初始化vscode功能 */
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider(SideBarView.YAPI_VIEW, slideWebview, {
      webviewOptions: {
    ...
    vscode.commands.registerCommand(Command.LOGOUT, () => {
      // 退出登录
      storage.clearAll()
      vscode.commands.executeCommand('setContext', ContextEnum.SHOW_TREE_VIEW, false)
      slideWebview.dove?.sendMessage(MsgType.LOGIN_STATUS, false)
    }),
    vscode.commands.registerCommand(Command.INSERT_TYPE, async ({ filePath, text }) => {
    ...
    vscode.commands.registerCommand(Command.FIX_ALL, async () => {
      ...
      if (!container.dove || !uri) return
      // 修复所有类型
      const url = uri.scheme + '://' + uri.fsPath
      const [fixs] = await container.dove.sendMessage<any[][]>(MsgType.FIX_ALL, url)
      ...
      for (let i = 0; i < diags.length; i++) {
        ...
        // 写入类型文件
        await vscode.commands.executeCommand(Command.INSERT_TYPE, commandInfo.arguments[0])
        // 写入文件变更
        changes.map((item) => {
          if (i !== 0 && item.newText.includes('type YapiResponse')) return
    ...
    vscode.workspace.onDidDeleteFiles((e) => {
      const deleteFiles = e.files?.map((file) => file.scheme + '://' + file.fsPath)
      const oldApiTypeList = storage.getStorage(StorageType.API_TYPE_LIST) as ApiTypeList
      const newApiTypeList = oldApiTypeList?.filter((file) => deleteFiles?.indexOf(file?.uri) === -1)
  ...
  async function diagnoseBaseInputFiles(files: string[]) {
     ...
     *  不存在列表中，诊断后添加
     */
    const fileTypeList = (await container?.dove?.sendMessage(MsgType.API_FILE_HANDLER, files)) || []
    const results = (fileTypeList as ApiTypeList[])?.[0]?.map((file) => file)
    const rootPath = vscode.workspace.getWorkspaceFolder(vscode.Uri.parse(results?.[0]?.uri))
    const replaceRex = rootPath?.uri?.scheme + '://' + rootPath?.uri?.fsPath
    const finalRet: ApiTypeList = results?.map((i) => ({ ...i, path: i?.uri?.replace(replaceRex, '') }))
    /**
     * 增量去重新诊断文件列表
     */
    const oldApiTypeList = storage.getStorage(StorageType.API_TYPE_LIST) as ApiTypeList
    let newApiTypeList: ApiTypeList = []
    if (!oldApiTypeList?.[0]) {
     ...
     * 2. 列表已加载，直接发送
     */
    if (storage.getStorage(StorageType.WEBVIEW_DONE) && Boolean(storage.getStorage(StorageType.LOGIN_INFO))) {
...
async function getApiFileList() {
  ...
  const filesFromApiDir = await vscode.workspace.findFiles('**/api/**/*.ts', 'node_modules/*', 100)
  filesFromApiDir.forEach((file) => fileList.push(file.scheme + '://' + file.fsPath))
  const filesFromApiFile = await vscode.workspace.findFiles('**/api.ts', 'node_modules/*', 100)
  filesFromApiFile.forEach((file) => fileList.push(file.scheme + ':' + file.fsPath))
  return fileList
}
