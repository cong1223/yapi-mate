...
import { getApiPositionList, getImportTypePosition, getYapiResponseInfo } from './parseAST'
...
dove.subscribe(MsgType.FIX_ALL, async (uri: string) => {
  const fixs = await provideCodeActions(store.get(uri).detail, true).catch((e) => {
...
async function validateTextDocument(textDocument: TextDocument): Promise<void> {
  ...
  for (let apiFn of apiFnList) {
    let mayTipsMessage = `${apiFn.apiPath} ${API_NOT_DEFINED} ${
      Number.isFinite(apiFn.paramTypeInsertPosition) ? '未定义参数类型' : ''
    } ${Number.isFinite(apiFn.fnRespTypeInsertPosition) ? '未定义返回值类型' : ''}
    ${Number.isFinite(apiFn.methodGenericInsertPosition) ? '未定义返回值类型' : ''}`
    ...
    diagStore.setCurDiagnostic(tipsMessage, {
      ...
      fnRespTypeInsertPosition: Number.isFinite(apiFn.fnRespTypeInsertPosition)
        ...
        : undefined,
      methodGenericInsertPosition: Number.isFinite(apiFn.methodGenericInsertPosition)
...
async function provideCodeActions(parms: CodeActionParams, isFixAll?: boolean): Promise<CodeAction[]> {
