...
import { YapiResponseName, YAPI_RESPONSE_TYPE } from '../../constant'
...
export function getApiPositionList(ast: ts.SourceFile, config?: any) {
  ...
  // 遍历所有节点 获取位置
  const transformer: ts.TransformerFactory<ts.Node> = (context: ts.TransformationContext) => {
...
function getApiNodeWhenCreateApi(node: ts.CallExpression, config?: any): false | FunctionStruct {
  const result: FunctionStruct = { apiPath: '', requestFn: 'any' }
  // request() 形式
  // @ts-ignore
  const hasMethodGeneric = node?.typeArguments?.length > 0
...
function getApiNode(node: FunctionNode, config?: any): false | FunctionStruct {
  ...
  // 参数是否进行类型定义
  const paramIsTyped = paramNodes.length === 0 ? true : getIsParamType(paramNodes[0] as ts.ParameterDeclaration)
  ...
  if (respIsTyped && paramIsTyped) return false
  // 存在参数和返回值未定义类型，读取函数体
  // isCallExpression ()=>request.get('xxx')
  const [callExpressionNode] = getTheChildNode<ts.CallExpression>(node, ts.isCallExpression)

  const hasResGeneric =
    callExpressionNode && callExpressionNode.typeArguments && callExpressionNode.typeArguments.length > 0
  if (hasResGeneric && paramIsTyped) return false

  if (config && config.responseType == YAPI_RESPONSE_TYPE.GENERIC) {
    if (!hasResGeneric) {
      result.methodGenericInsertPosition = getResponseGenericInsertPosition(callExpressionNode)
    }
  } else {
    if (!respIsTyped) {
  ...
  if (bodyNode) {
    ...
    if (returnNode) {
      const [callExpressionNode] = getTheChildNode<ts.CallExpression>(returnNode, ts.isCallExpression)
      if (callExpressionNode) {
        const apiPath = getApiUrlFromExpressionNode(callExpressionNode)
        if (apiPath) {
          result.apiPath = apiPath
...
function isCreateApi(node: ts.Node): boolean {
  return (
    (ts.isCallExpression(node) && (node.expression as any)?.escapedText === 'createApi') ||
    (getTheChildNode<ts.PropertyAccessExpression>(node, ts.isPropertyAccessExpression)?.[0]?.expression as any)
 ...
 * @description 获取指定子节点
 */
function getTheChildNode<T = ts.Node>(parentNode: ts.Node, jugeFn: Function): T[] {
  const gather: T[] = []
 ...
 */
function getApiUrlFromExpressionNode(node: ts.ExpressionStatement | ts.CallExpression) {
  // ts.isStringLiteral
  let strNodes: any[] = []
  const identifierNode = getTheChildNode<ts.StringLiteral>(node, ts.isToken)
  if (identifierNode) {
    strNodes.push(...identifierNode.map((node) => getUrlFromStringNode(node as ts.StringLiteral)))
  }
  const strNode = getTheChildNode<ts.StringLiteral>(node, ts.isStringLiteral)
  if (strNode) {
    strNodes.push(...strNode.map((node) => getUrlFromStringNode(node as ts.StringLiteral)))
  }
  // 携带变量
  const [variableNode] = getTheChildNode(node, ts.isTemplateExpression)
  if (variableNode) {
    // @ts-ignore
    const nodes = variableNode?.templateSpans?.map((node) => getTheChildNode(node, ts.isTemplateTail)?.[0])
    strNodes.push(...nodes?.map((node: any) => getUrlFromStringNode(node as ts.StringLiteral)))
  }
...
export async function getImportTypePosition(ast: ts.SourceFile) {
  const importNodes = getTheChildNode<ts.ImportDeclaration>(ast, ts.isImportDeclaration)
...
export function getYapiResponseInfo(ast: ts.SourceFile) {
  ...
  for (let node of importNodes) {
    ...
    if (bindingNode) {
      for (let node of getTheChildNode<ts.ImportSpecifier>(bindingNode, ts.isImportSpecifier)) {
        if (node.name.escapedText === YapiResponseName) {
          return {
            type: 'useOld',
            position: 0,
          }
    ...
    if (name?.escapedText === YapiResponseName) {
      return {
        type: 'useOld',
        position: 0,
      }
  ...
  // 查找命名变量
  const typeAliasNode = getTheChildNode<ts.TypeAliasDeclaration>(ast, ts.isTypeAliasDeclaration)
  const interfaceNode = getTheChildNode<ts.InterfaceDeclaration>(ast, ts.isInterfaceDeclaration)
  for (let node of [...typeAliasNode, ...interfaceNode]) {
    if (node.name.escapedText === YapiResponseName) {
      return {
        type: 'useOld',
        position: 0,
...
function getFnName(node: ts.CallExpression): string {
  const [pNode] = getTheChildNode<ts.PropertyAccessExpression>(node, ts.isPropertyAccessExpression)
  if (pNode) {
    // 形式 request.get
    return (
