...
import { CodeAction, CodeActionKind, CodeActionParams, Diagnostic, DiagnosticSeverity } from 'vscode-languageserver'
import { Command, YapiResponseName } from '../../constant'
...
async function checkApiTypeExist(typeFilePath: string, apiData: ApiInterface, key: keyof ApiInterface) {
  try {
...
export async function quickfix(dove: Dove, parms: CodeActionParams, isFixAll?: boolean): Promise<CodeAction[]> {
  ...
  for (let diag of diagnostics) {
    ...
    if (!current) continue
    if (diag.severity === DiagnosticSeverity.Information && diag.message.includes(API_NOT_DEFINED)) {
      ...
      const typeFilePath = getFileCurrentWorkSpace(parms.textDocument.uri, 'types.ts')
      ...
      if (current.paramTypeInsertPosition) {
        ...
        fixList.push({
          ...
          newText: `: Partial<${apiData.reqBodyType ? apiData.reqBodyTitle : apiData.reqQueryTitle}> `,
        })
        anyFixList.push({
          range: {
            start: current.paramTypeInsertPosition,
      ...
      if (current.methodGenericInsertPosition) {
        let exist = await checkApiTypeExist(typeFilePath, apiData, 'resBodyDataTitle')
        ...
        fixList.push({
          ...
          newText: apiData.resBodyDataType ? `<${apiData.resBodyDataTitle}>` : '<any>',
        })
        anyFixList.push({
          range: {
            start: current.methodGenericInsertPosition,
      ...
      if (current.fnRespTypeInsertPosition) {
        let exist = await checkApiTypeExist(typeFilePath, apiData, 'resBodyTitle')
        ...
        fixList.push({
          ...
          newText: apiData.resBodyType ? `: ${YapiResponseName}<${apiData.resBodyTitle}> ` : ': any ',
        })
        anyFixList.push({
          range: {
            start: current.fnRespTypeInsertPosition,
          ...
          fixList.push({
            ...
            newText: `\nimport type { ${[
              current.paramTypeInsertPosition
                ...
                : '',
              current.fnRespTypeInsertPosition && apiData.resBodyType ? apiData.resBodyTitle : '',
              current.methodGenericInsertPosition && apiData.resBodyDataType ? apiData.resBodyDataTitle : '',
            ]
              .filter(Boolean)
              .join(', ')} } from './types'`,
          })
