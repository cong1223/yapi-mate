 ...
 *
 */
const getApiUrl = (data: DetailData) => {
  return (
...
export function data2Type(data: DetailData) {
  ...
  const reqQueryType = data.req_query
  ...
  const reqBodyType = data.req_body_other
    ? formatInterfaceComment(data, 'post请求体') +
      reqBody2type(interfaceName, JSON.parse(data.req_body_other))
    : '';
  const resBodyType = data.res_body
    ? formatInterfaceComment(data, '响应体') +
      resBody2type(interfaceName, JSON.parse(data.res_body))
    : '';
  const resBodyDataType = data.res_body ? formatInterfaceComment(data, '响应体') + resBodyData2type(interfaceName, JSON.parse(data.res_body).properties.data) : ''
